...
**Video Reference** owns the persistent public tag `<Video 1>`. **Driving Audio** is persistent downstream conditioning and owns no `<Audio N>` prompt tag. Writer validates these scopes in the semantic plan, generated chunk, and chunk-refinement paths.
