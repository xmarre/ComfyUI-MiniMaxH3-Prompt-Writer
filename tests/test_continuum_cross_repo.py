...
class ContinuumCrossRepositoryContractTests(unittest.TestCase):
    ...
    def test_v34_public_sampler_socket_contract_matches_frontend_discovery(self):
        base_source = (self.source_root / "v3" / "nodes.py").read_text(encoding="utf-8")
        v34_source = (self.source_root / "v3" / "driving_nodes.py").read_text(encoding="utf-8")
        ...
        # Current upstream V3.4 inherits the Production facade's three
        # persistent image-reference sockets. Writer discovery is intentionally
        # tolerant of extended facades, but the pinned upstream contract itself
        # must not be assumed to expose non-upstream Reference Image 4–8 sockets.
        self.assertNotIn("for index in range(4, 9):", v34_source)
        self.assertIn('optional["reference_video_1"] = (', v34_source)
        self.assertIn('"IMAGE",', v34_source[v34_source.index('optional["reference_video_1"] = ('):])
        self.assertIn('optional["driving_audio"] = (', v34_source)
        self.assertIn('"AUDIO",', v34_source[v34_source.index('optional["driving_audio"] = ('):])
