...
class DownstreamReferenceInventoryTests(unittest.TestCase):
    ...
    def test_video_reference_owns_video_one_and_driving_audio_owns_no_audio_tag(self):
        value = normalize_downstream_reference_inventory(
            downstream_inventory(0, video_reference=True, driving_audio=True)
        )
        self.assertEqual(
            [(item["role"], item.get("tag")) for item in value["items"]],
            [("video_reference", "<Video 1>"), ("driving_audio", None)],
        )
