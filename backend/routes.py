...
async def _generate_continuum(
    ...
    try:
        ...
        expected_references = {
            str(asset["reference"])
            for asset in planner_assembled["input"]["media_manifest"].get("assets", [])
        ...
        try:
            ...
            try:
                ...
                raise ModelError(
                    second_error.code,
                    "The sequence planner remained structurally invalid after one narrow repair.",
                    {
        ...
        sequence = sequence_result(settings, plan, prompts)
...
async def refine(request: web.Request) -> web.Response:
    ...
    if body.get("seed") is not None and (not isinstance(body["seed"], int) or isinstance(body["seed"], bool) or body["seed"] < 0):
        return _error("INVALID_REQUEST", "Seed must be a non-negative integer.", status=400)
    continuum_refinement: tuple[list[str], int, dict[str, Any]] | None = None
    try:
        if target == GENERATION_TARGET_CONTINUUM:
            assembled, sequence_prompts, chunk_index, sequence_plan = assemble_continuum_refinement(body)
            continuum_refinement = (sequence_prompts, chunk_index, sequence_plan)
        else:
    ...
    try:
        ...
        if continuum_refinement is not None:
            sequence_prompts, chunk_index, sequence_plan = continuum_refinement
            try:
