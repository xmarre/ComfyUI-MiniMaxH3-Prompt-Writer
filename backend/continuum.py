...
def validate_saved_downstream_inventory(
    ...
    if downstream_inventory_identity(saved) != downstream_inventory_identity(active):
        raise ContinuumError(
            ...
            (
                "The selected V3.4 sampler conditioning inventory changed since this Continuum sequence was generated. "
...
def validate_continuum_mode_topology(mode: str, request_input: dict[str, Any]) -> None:
    ...
    if actual != required or reference_mode_mismatch:
        ...
        if reference_mode_mismatch:
            message = (
                "T2VA does not match the selected V3.4 sampler conditioning. T2VA requires no First/Last "
            ...
            message = (
                f"{mode} does not match the selected V3.4 sampler's temporal keyframe wiring. "
...
def persistent_reference_tags(request_input: dict[str, Any], *, chunks: int) -> set[str]:
    ...
    return {
        str(item["tag"])
        for item in tagged
        if item["role"] in {"reference_image", "video_reference"}
    }
    ...
    *,
    ...
    for item in inventory["items"]:
        ...
        if not tag:
            continue
        role = item["role"]
        if role in {"reference_image", "video_reference"}:
            allowed.add(str(tag))
...
def _preflight_body_references(base_input: dict[str, Any], texts: list[str]) -> None:
    ...
    if missing:
        ...
        raise ContinuumError(
            ...
            f"{tag} is used by the Continuum request but is not declared by the selected H3 Continuum V3.4 sampler inventory.",
...
def assemble_continuum_plan_request(body: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(body.get("downstream_reference_inventory"), dict):
        raise ContinuumError(
            ...
            "H3 Continuum requires the authoritative downstream conditioning inventory from the selected V3.4 sampler.",
            {
                "field": "downstream_reference_inventory",
                "suggestion": (
                    "Select one H3 Continuum Sampler V3.4 and retry. An explicitly empty inventory is valid "
    ...
    user = (
        ...
        "Every inventory entry with an explicit public tag owns that exact H3 prompt identity. Reference Images and "
        "Video Reference are persistent across chunks. Without Reference Images, a tagged First Frame is opening-chunk-only "
        "and a tagged Last Frame is final-chunk-only; do not place those scoped keyframe tags in the shared preamble of a "
        "multi-chunk sequence. Driving Audio is persistent conditioning and owns no <Audio N> tag. Any item marked not "
