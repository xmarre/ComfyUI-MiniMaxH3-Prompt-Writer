...
def normalize_downstream_reference_inventory(value: Any) -> dict[str, Any]:
    ...
    if reference_images:
        if any(item is not None and item.get("tag") for item in (first_frame, last_frame)):
            raise ValueError(
                "First/Last Frame must not own public <Picture N> tags when V3.4 Reference Images are active; "
        ...
        if actual_keyframe_tags != expected_keyframe_tags:
            raise ValueError(
                "Without V3.4 Reference Images, active First/Last Frame inputs must own compact public "
...
def reference_policy(request_input: dict[str, Any]) -> ReferencePolicy:
    ...
    if continuum and "downstream_reference_inventory" in request_input:
        required = {
            ...
            if tag.startswith("<Picture ") or tag == "<Video 1>"
        }
    allowed_audio = {
        str(asset["reference"])
        for asset in assets
